...
| <!-- aip-exception:create-instance-spec --> `CreateInstanceRequest.spec` | `CreateInstanceRequest` now supports canonical `Instance instance = 5` and keeps `CreateInstanceSpec spec = 2` for existing clients. Exactly 1 of `spec` or `instance` is accepted. | AIP Create normally uses `{resource} resource` body and `{resource}_id`; body `name` is ignored. | <!-- aip-classification:permanent-v1alpha1-exception --> Permanent v1alpha1 compatibility field. | New clients should send `instance`; existing clients may keep using `spec`. The server composes `Instance.name` from `instance_id` and ignores body `instance.name`. |
...
| <!-- aip-exception:operational-surfaces --> Console, onboarding, and SQL surfaces | Console configuration, onboarding and setup streams, and ad hoc SQL query methods use custom request and response shapes. | These are operational methods rather than resource CRUD. | Intentional compatibility exception. | Keep. Apply AIP resource patterns only if these surfaces introduce durable resources. |
| <!-- aip-exception:scalar-filter-colon --> Scalar filter `:` semantics | Querylane list filters use `field:"text"` for case-insensitive scalar substring matching. Filter field paths still match the public resource fields exactly. | AIP-160 defines `:` as the has operator, primarily for repeated or map fields. | Intentional v1alpha1 compatibility exception. | Keep the established scalar-search grammar in v1alpha1. New proto declarations must mark the deviation with `aip.dev/not-precedent`; strict AIP surfaces should not copy it. |
...
The parallel pagination work remains scoped to AIP list pagination and token semantics. These exceptions are pre-existing API compatibility choices or migration candidates. Folding them into pagination work would mix pagination fixes with breaking or broad proto-shape changes, increasing review risk without improving pagination behavior.
