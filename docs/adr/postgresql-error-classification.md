...
Callers select 1 of 2 explicit profiles:
...
SQLSTATE input is trimmed, uppercased, and accepted only when it contains 5 ASCII letters or digits. Valid unknown codes preserve their code and class and use an available class fallback. Malformed codes remain internal and omit code, class, and condition from client metadata.
