# Dependency upgrade: Playwright 1.62.0-alpha-2026-07-03 to 1.62.0-alpha-2026-07-06

## Summary
- Ecosystem: npm and Bun
- Manifest and lockfiles: `frontend/package.json`, `frontend/bun.lock`
- Dependency relation: direct dev dependencies `playwright` and `playwright-core`; `playwright` depends on matching `playwright-core`
- Request: Ben, with explicit direction to stay on the latest `next` prerelease channel (`rc`, `alpha`, or `beta`)

## Version path
Every published stable version from current exclusive to target inclusive was researched; installing every version was unnecessary. No stable versions were released between these 2 prereleases. The applicable npm `next` prerelease path is:

| Step | From | To | SemVer class or non-SemVer scale | Source | Migration/breakage | Action |
|---|---:|---:|---|---|---|---|
| 1 | 1.62.0-alpha-2026-07-03 | 1.62.0-alpha-2026-07-04 | prerelease alpha | npm registry distribution and version metadata | No official release notes for daily alpha. Change is prerelease-only and inherits alpha churn risk. | Keep the existing Playwright configuration; verify installation, type check, and end-to-end configuration load. |
| 2 | 1.62.0-alpha-2026-07-04 | 1.62.0-alpha-2026-07-05 | prerelease alpha | npm registry distribution and version metadata | No official release notes for daily alpha. Change is prerelease-only and inherits alpha churn risk. | Keep the existing Playwright configuration; verify installation, type check, and end-to-end configuration load. |
...
- Do not fall back to latest stable `1.61.1`; the user explicitly prefers the latest prerelease (`rc`, `alpha`, or `beta`).
...
Repository dependents: e2e CLI scripts, `frontend/e2e/playwright.config.ts`, `@vitest/browser-playwright`, `@axe-core/playwright`, Lighthouse script importing `chromium` from `playwright`
...
Plugins and adapters: Vitest browser Playwright provider, axe-core Playwright integration.

## Non-SemVer scale
Release cadence: daily prerelease alpha builds plus monthly stable releases.
Change volume: unknown per daily alpha; target tarballs are slightly larger than `1.61.1` and newer than the existing alpha by 3 daily builds.
Diff size: package pins plus lockfile entries and this report.
API churn: medium because this is the prerelease `next` channel.
Effort: low for repository integration because the current no-retry configuration loads and tests pass.
Danger and blast radius: end-to-end and browser-test tooling only; no production runtime dependency, but CI may need a fresh browser cache after alpha changes.

## Security notes
| Advisory | Source | Reachability and exploitability | Fixed version | Decision |
|---|---|---|---|---|
| None found for `playwright@1.62.0-alpha-2026-07-06` | OSV query, GitHub advisory query | Development and test package; no known advisory for target | Not applicable | Accept prerelease risk by user preference |
| None found for `playwright-core@1.62.0-alpha-2026-07-06` | OSV query, GitHub advisory query | Development and test package; no known advisory for target | Not applicable | Accept prerelease risk by user preference |
...
## Risk gate
Decision: apply now
Reason: prerelease and non-SemVer risk, but this is development and test tooling, the repo was already on a Playwright alpha, and the user explicitly prefers the latest prerelease channel over stable.
...
Tests: `bun run test` passed (118 unit files, 1047 tests; 56 integration files, 296 tests). `QUALITY_BASE_REF=origin/main bun run test:browser:changed -- --reporter=verbose` passed (4 browser files, 22 tests). `bunx vitest run --config vitest.browser.all.config.ts --reporter=verbose` passed (34 browser files, 204 tests). `CI=1 bun run test:e2e:list` passed (83 end-to-end tests listed). The direct CI configuration assertion passed with `retries: 0`, `trace: "retain-on-failure"`, `video: "off"`, and no `failOnFlakyTests`. Linux browser baselines were regenerated in Docker for the rebased instance overview surface.
Build, vet, and security scan: `bun install --frozen-lockfile --ignore-scripts` passed. `bunx playwright --version` reported `Version 1.62.0-alpha-2026-07-06`. `bun audit --json` reported existing unrelated advisories in `js-yaml`, `tmp`, and `uuid` from `@lhci/cli` and `@changesets`; no Playwright advisory was found in OSV or GitHub advisory checks.
