# Dependency upgrade: Vitest 4.1.9 to 5.0.0-beta.5

## Summary
- Ecosystem: JavaScript and Bun frontend workspace
- Manifest and lockfiles: `frontend/package.json`, `frontend/bun.lock`
- Dependency relation: direct dev dependencies `vitest`, `@vitest/browser`, `@vitest/browser-playwright`, `@vitest/ui`
- Request: user requested latest Vitest 5 beta migration and benchmark
...
|---|---:|---:|---|---|---|---|
| 1 | 4.0.0 | 4.0.18 | patch series | GitHub releases and package registry | No repo-specific migration found beyond normal patch fixes. | No code change. |
| 2 | 4.0.18 | 4.1.9 | minor and patch series | GitHub releases and package registry | No repository-specific migration found; latest stable before v5 is `4.1.9`. | Used as benchmark baseline. |
| 3 | 4.1.9 | 5.0.0-beta.1 | prerelease major | Vitest 5 migration guide | Requires Node >= 22.12 and Vite >= 6.4. Major behavior changes include default `clearMocks`, stricter hoisted mocks, benchmark API rewrite, browser locator and text strictness, report and artifact moves, configuration lookup changes, and browser screenshot and orchestrator changes. | Audited repository patterns and ran tests. |
| 4 | 5.0.0-beta.1 | 5.0.0-beta.5 | prerelease patches | GitHub releases and package registry | Latest beta includes browser orchestrator readiness fixes, session-bound browser runner URLs, worker and concurrency ID changes, and configuration lookup changes. | Target remains `5.0.0-beta.5`. |
...
- The `toThrow("")` and `toThrowError("")` audit found no affected assertions.

Behavior and configuration changes:
- Node prerequisite satisfied locally: Node `v26.0.0`.
- Vite prerequisite satisfied through lockfile: `vite@8.0.16`.
- `clearMocks` v5 default is accepted and now explicit in unit, integration, and browser configurations.
- Browser locator strictness is accepted and now explicit through `browser.locators.exact: true`; browser tests pass on v5 without compatibility fallback.
- Vitest v5 generated artifact root is covered by existing `frontend/.vitest` ignore.
- Configuration is already explicit at every package script call site, so removing v5 parent-directory configuration lookup does not affect local scripts.
...
Repository dependents: frontend unit, integration, browser, changed-test, and CI scripts
Peers: `@vitest/browser-playwright@5.0.0-beta.5`, `@vitest/ui@5.0.0-beta.5`, `happy-dom`, `@types/node`, `vite`  
Plugins and adapters: `@vitest/browser@5.0.0-beta.5`, `@vitest/browser-playwright@5.0.0-beta.5`, `vitest-browser-react@2.2.0`
...
API churn: medium, mostly browser mode, benchmarks, reports, mocks, and configuration lookup
Effort: low after audit because tests already conform  
Danger and blast radius: medium; test infrastructure only, but browser mode validates customer-facing surfaces

## Security notes
| Advisory | Source | Reachability and exploitability | Fixed version | Decision |
|---|---|---|---|---|
| GHSA-w5hq-g745-h8pq | Bun audit | Existing transitive dependency from `@lhci/cli` to `uuid`, not introduced by Vitest v5. | `uuid >=11.1.1` | Out of scope for Vitest migration; track separately. |
| GHSA-52f5-9888-hmc6, GHSA-ph9p-34f9-6g65 | Bun audit | Existing transitive dependency from `@lhci/cli` to `tmp`, not introduced by Vitest v5. | patched `tmp` release unavailable through current parent path | Out of scope for Vitest migration; track separately. |
| GHSA-h67p-54hq-rp68 | Bun audit | Existing transitive dependency from `@lhci/cli`, `shadcn`, and `@changesets/cli`, not introduced by Vitest v5. | `js-yaml >=3.15.0` | Out of scope for Vitest migration; track separately. |

## Benchmark
Machine: local macOS arm64, Node `v26.0.0`, Bun `1.3.14`. Benchmarks used `hyperfine`; unit and integration used 5 measured runs with 1 warmup. Browser v5 used 3 measured runs with 1 warmup. Browser v4 was a single compatibility run because it failed.
...
Build, vet, and security scan: `bun audit` reports pre-existing non-Vitest transitive advisories from `@lhci/cli`, `shadcn`, and `@changesets/cli`; not introduced by this migration.
