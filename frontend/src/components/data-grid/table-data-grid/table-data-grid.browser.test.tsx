...
import { createCellSelectionStore } from "@/components/data-grid/table-data-grid/cell-selection-state";
...
function columnLayoutProps(columns: TableResultColumn[]) {
  return {
    cellSelectionStore: createCellSelectionStore(),
...
test("cell selection summary exposes copy, clear, and live status", async () => {
  ...
  await expect
    .element(page.getByRole("status", { name: "Cell selection" }))
    .toHaveTextContent("6 cells selected in 2 rows by 3 columns.");

  await page.getByRole("button", { name: "Copy selected cells" }).click();
  expect(onCopyCellSelection).toHaveBeenCalledOnce();

  await page.getByRole("button", { name: "Clear cell selection" }).click();
  await expect.element(page.getByText("6 cells · 2×3")).not.toBeInTheDocument();
  await expect
    .element(page.getByRole("status", { name: "Cell selection" }))
    .toHaveTextContent("Cell selection cleared.");
});
...
test("grid and cell selection expose accessible names and state", async () => {
  renderForeignKeyReferenceGrid(
    "h-[620px] w-[1120px] rounded-2xl border border-border bg-background p-6 text-foreground"
  );
