...
describe("DataCell binary values", () => {
  it("keeps binary grid cells length-only without offering a preview", () => {
    const column = create(TableResultColumnSchema, {
      columnName: "avatar",
    ...
    expect(
      screen.queryByRole("button", { name: "Preview avatar binary data" })
    ).toBeNull();
  });
});
