...
const PILL_BASE =
  "inline-flex h-[18px] items-center justify-center gap-0.5 rounded border px-1.5 font-mono text-[0.625rem] leading-none tracking-[0.06em]";
    ...
    <Tooltip>
      <TooltipTrigger render={<span className={className} />}>
        ...
        {state !== "none" && count !== undefined ? (
          <span className="text-[0.5625rem] opacity-80">
...
export function HeldPillStrip({
  ...
  if (ordered.length === 0) {
    return (
      <span className="font-mono text-[0.6875rem] text-muted-foreground/50 italic">
        no privileges
      </span>
    );
  }
...
export function FilterChip({
  ...
  return (
    <Button
      aria-pressed={active}
      className="h-[22px] rounded-full px-2 font-mono text-[0.625rem] tracking-[0.06em]"
      onClick={onToggle}
      size="xs"
      type="button"
...
export function CountPill({
  ...
  return (
    <span className="inline-flex h-[18px] items-center gap-1 rounded-full border border-border bg-secondary px-[7px] font-medium font-mono text-[0.6875rem] text-muted-foreground tracking-[0.02em]">
